...
def track_max_length_rows_of_a_csv_file(func):
    def wrapper_func(file_path):
        ...
        longest_row = [i for i, row in enumerate(csv_content) if len(row) == longest_row]
...
def track_min_length_rows_of_a_csv_file(func):
    def wrapper_func(file_path):
        ...
        shortest_row = [i for i, row in enumerate(csv_content) if len(row) == shortest_row]
